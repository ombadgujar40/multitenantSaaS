...
export const getTasks = async (req, res) => {
  ...
  try {
    if (role == "admin") {
      const resp = await prisma.task.findMany({ where: {project: {orgId: Number(organisation)}}, select: { id: true, title: true, description: true, status: true, createdAt: true, assignedToId: true, assignedTo: true, dueDate: true, projectId: true, project: true} })
